...
class PlasmaModelingDataSource(DataSource):
    """Data source for reading and writing DryResist plasma simulation data."""
    ...
    def __init__(
        ...
        kind: DatasetKind | str = DatasetKind.DRYRESIST,
    ...
    def read_file(self, dirname: str) -> PlasmaModelingExtractedDataInMemory:
        """Read DryResist plasma simulation data from a directory.

        Args:
            dirname: Name of the simulation directory (e.g., 's_001')

        Returns:
            PlasmaModelingExtractedDataInMemory containing processed simulation data,
            and metadata (PlasmaModelingMetadata).
        ...
        """
        case_dir = self.input_dir / dirname

        # Load STL geometry
        stl_path = self.path_getter.geometry_path(case_dir)
        ...
        parameters_path = self.path_getter.parameters_path(case_dir)
        simulation_params = {}
        if parameters_path.exists():
            with open(parameters_path, "r") as f:
                simulation_params = yaml.safe_load(f)
        ...
        metadata = PlasmaModelingMetadata(
            ...
            simulation_params=simulation_params,  # Store loaded parameters
        )

        return PlasmaModelingExtractedDataInMemory(
            stl_polydata=stl_polydata,
            surface_polydata=surface_polydata,
            volume_unstructured_grid=None,  # No volume data for DryResist
            metadata=metadata,
        )
    ...
    def _write_impl_temp_file(
        self,
        data: (
            PlasmaModelingNumpyDataInMemory | PlasmaModelingZarrDataInMemory
        ),
        output_path: Path,
    ) -> None:
        ...
        """
        if self.serialization_method == "numpy":
            if not isinstance(data, PlasmaModelingNumpyDataInMemory):
                raise TypeError(
                    "Expected PlasmaModelingNumpyDataInMemory for numpy serialization"
                )
            self._write_numpy(data, output_path)
        elif self.serialization_method == "zarr":
            if not isinstance(data, PlasmaModelingZarrDataInMemory):
                raise TypeError(
                    "Expected PlasmaModelingZarrDataInMemory for zarr serialization"
                )
    ...
    def _write_numpy(
        self, data: PlasmaModelingNumpyDataInMemory, output_path: Path
    ) -> None:
    ...
    def _write_zarr(
        self, data: PlasmaModelingZarrDataInMemory, output_path: Path
    ) -> None:
