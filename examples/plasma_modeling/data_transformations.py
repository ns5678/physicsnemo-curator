...
import numpy as np
import zarr
from plasma_geometry_data_processors import (
    default_geometry_processing_for_plasma,
)
from plasma_global_params_data_processors import (
    default_global_params_processing_for_plasma,
)
from plasma_surface_data_processors import (
    default_surface_processing_for_plasma,
)
from plasma_utils import to_float32
from schemas import (
    PlasmaModelingExtractedDataInMemory,
    PlasmaModelingNumpyDataInMemory,
    PlasmaModelingNumpyMetadata,
    PlasmaModelingZarrDataInMemory,
...
class PlasmaModelingNumpyTransformation(DataTransformation):
    """Transforms Plasma Modeling data for NumPy storage format (legacy support)."""

    def __init__(self, cfg: ProcessingConfig):
        super().__init__(cfg)

    def transform(
        self, data: PlasmaModelingExtractedDataInMemory
    ) -> PlasmaModelingNumpyDataInMemory:
        ...
        Args:
            data: Plasma Modeling extracted data in memory

        Returns:
            Data formatted for NumPy storage with basic metadata
        """
        # Create minimal metadata
        numpy_metadata = PlasmaModelingNumpyMetadata(
            filename=data.metadata.filename,
        )

        return PlasmaModelingNumpyDataInMemory(
            ...
            surface_variable_names=data.surface_variable_names,
            global_params_values=to_float32(data.global_params_values),
            global_params_reference=to_float32(data.global_params_reference),
        )


class PlasmaModelingSTLTransformation(DataTransformation):
    """Transforms STL geometry data for Plasma Modeling."""
    ...
    def transform(
        ...
        """Transform STL data for Plasma Modeling.

        Applies default geometry processing to extract vertices, faces, areas,
        and centers from the STL polydata.
        """

        # Apply default geometry processing
        # This ensures geometry data (vertices, faces, areas, centers) are present
        data = default_geometry_processing_for_plasma(data)
...
class PlasmaModelingSurfaceTransformation(DataTransformation):
    """Transforms surface data for Plasma Modeling."""
    ...
    def __init__(
        ...
        self.logger.info(
            f"Initializing PlasmaModelingSurfaceTransformation with "
            f"surface_variables: {list(surface_variables.keys())} "
            f"and {len(surface_processors) if surface_processors else 0} processors"
        )

    def transform(
        self, data: PlasmaModelingExtractedDataInMemory
    ) -> PlasmaModelingExtractedDataInMemory:
        """Transform surface data for Plasma Modeling.

        Extracts surface fields, mesh centers, normals, and areas from VTP data.
        """

        if data.surface_polydata is not None:
            # Apply default surface processing
            # This ensures surface data (mesh centers, normals, areas, fields) are present
            data = default_surface_processing_for_plasma(
...
class PlasmaModelingGlobalParamsTransformation(DataTransformation):
    """Transforms global parameters for Plasma Modeling.

    Extracts global parameters from parameters.yaml and creates
    reference arrays for normalization.
    """
    ...
    def transform(
        ...
        """Transform global_params data for Plasma Modeling.

        Processes global parameter references from config and extracts
        actual values from simulation parameters.yaml file.
        """
        # Apply default processing to set up reference arrays from config
        data = default_global_params_processing_for_plasma(
            data, self.global_parameters
        )

        # Apply processors to extract actual values from simulation data
...
class PlasmaModelingZarrTransformation(DataTransformation):
    """Transforms Plasma Modeling data for Zarr storage format."""
    ...
    def __init__(
        ...
        chunks_per_shard: int = 1000,  # Number of chunks per shard
    ):
        super().__init__(cfg)
    ...
    def transform(
        ...
        Organizes data and applies compression settings.

        Args:
            data: Plasma Modeling extracted data

        Returns:
            Data formatted for Zarr storage with compression and chunking
        """
        return PlasmaModelingZarrDataInMemory(
